...
4. **Send** via TL's Microsoft Graph API Outlook account
   - To: client contact email (from `requirements`)
...
7. **For GeBIZ**: insert row into `interview_tracker` table
...
## DB Tables Touched
- Read: `submissions`, `candidates`, `requirements`
- Write: `submissions` (update), `candidate_details` (update status), `interview_tracker` (insert for SG tenders)
